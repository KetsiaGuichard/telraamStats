...
create_and_train_model <- function(data, target, base_vars) {
  ...
  data_train <- data_rf[is_train, ] %>% mutate(imputed = "original") %>% na.omit()
  data_test <- data_rf[!is_train, ] %>% mutate(imputed = "imputed") %>% select(-y)  %>% na.omit()
...
impute_missing_data <- function(data, sensors_name = NULL, transport_type = "vehicle") {
  ...
  original_data <- data
  print(table(data_complete$segment_name))
  print(table(original_data$segment_name))
  # Define constants
  BASE_VARS <- c("day_of_month", "hour", "weekday", "month", "year", "vacation", "season", "week_number", "segment_name", "date")
  ...
  data_complete <- left_join(
    data_complete,
    original_data %>% select(segment_name, date,hour, all_of(vars_to_add)),
...
# Example usage:
# Impute missing data for the transport type 'car'
