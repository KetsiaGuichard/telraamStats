...
rolling_hourly <- function(data, val_reel, windows) {
  ...
  for (window in windows) {
    ...
    errors_mean <- val_reel  %>% left_join(res_imput, by = "date", suffix = c("", "_chap")) %>% 
      mutate(error = y - y_chap) %>% 
      summarise(mae = mean(abs(error), na.rm = TRUE), rmse = sqrt(mean(error^2, na.rm = TRUE)))
    
    errors_median <- val_reel  %>% left_join(res_imput, by = "date", suffix = c("", "_chap")) %>% 
      mutate(error = y - y_chap) %>% 
      summarise(mae = mean(abs(error), na.rm = TRUE), rmse = sqrt(mean(error^2, na.rm = TRUE)))
...
results_combined <- rolling_hourly(data_imput, val_reel, windows)
...
p1 <- ggplot(results_combined, aes(x = window, y = mae_mean)) +
  geom_line() +
  labs(
...
p2 <- ggplot(results_combined, aes(x = window, y = na_mean)) +
  geom_line() +
  labs(
...
p5 <- crit_MA_data %>% group_by(window) %>% summarise(mae_mean = mean(mae_mean), rmse_mean = mean(rmse_mean), na_mean = mean(na_mean), mae_median = mean(mae_median), rmse_median = mean(rmse_median), na_median = mean(na_median)) %>% ggplot(aes(x = window, y = mae_mean)) +
  geom_line() +
  labs(
...
p6 <- crit_MA_data %>% group_by(window) %>% summarise(mae_mean = mean(mae_mean), rmse_mean = mean(rmse_mean), na_mean = mean(na_mean), mae_median = mean(mae_median), rmse_median = mean(rmse_median), na_median = mean(na_median)) %>% ggplot(aes(x = window, y = na_mean)) + 
  geom_line() +
  labs(
    title = "Comparaison du nombre de NA en fonction de la taille de la fenêtre",
    x = "Taille de la fenêtre",
...
crit_MA_data[which.min(crit_MA_data$mae_mean),]
...
```{r}
#Tester l'algorithme sur les sensors puis calculer la MAE et tracer  pour chaque valeur de window
windows = 2:50
results <- data.frame(sensor = character(), window = integer(), MAE = numeric(),NB_NA = numeric())


for (sensor in sensors) {
  sensor_name <- sensor$segment_name[1]
  ...
  val_reel <- data_imput_list$temoin %>% filter(date %in% date_imput) %>% select(date, vehicle, hour, weekday)

  for (window in windows) {
    # Impute missing values
    imputed_data <- impute_NA_vehicle(data_imput, window = window)
    imputed_data <- imputed_data %>% filter(imputed == "imputed")

    # Calculate the MAE
    crit <- imputed_data %>% inner_join(val_reel, by = "date") %>%
      mutate(error = abs(vehicle.x - vehicle.y))
    mae <- mean(crit$error, na.rm = TRUE)

    # Calculate the number of NA after imputation
    nb_na <- sum(is.na(imputed_data$vehicle))

    # Store the results
    results <- rbind(results, data.frame(sensor = sensor_name, window = window, MAE = mae, NB_NA = nb_na))
  }
}


#Add the mean of all sensor to results with "Moyenne des capteurs" as name for the sensor
mean_results <- results %>%
  group_by(window) %>%
  summarise(MAE = mean(MAE, na.rm = TRUE),
            NB_NA = sum(NB_NA)) %>%
  mutate(sensor = "Moyenne des capteurs")

results <- rbind(results, mean_results)


# Plot the results
ggplot(results, aes(x = window, y = MAE)) +
  geom_line() +
  labs(title = "MAE for Different Window Sizes", x = "Window Size", y = "MAE") +
  theme_minimal() +
