...
```{r, echo=FALSE, "Comparaison de la moyenne des vélos puis des véhicules en fonction du seuil de pluie pendant les heures d'affluence le matin (7h et 8h), en semaine et pendant la période scolaire"}
# Initialisation des seuils et du tableau pour les résultats
seuils <- seq(0.1, 6, by = 0.5)
...
```{r, echo=FALSE, "Comparaison de la moyenne des vélos puis des véhicules en fonction du seuil de pluie pendant les heures d'affluence le SOIR (17h et 18h), en semaine et pendant la période scolaire"}
# Initialisation des seuils et du tableau pour les résultats
seuils <- seq(0.1, 6, by = 0.5)
resultats <- data.frame(seuil = numeric(), condition = character(), moyenne_vehicule = numeric(), p_value = numeric())
...
```{r, echo=FALSE, "Comparaison de la moyenne des vélos puis des véhicules en fonction du seuil de pluie pendant les heures d'affluence le matin (7h et 8h), en semaine et pendant la période non scolaire"}
# Initialisation des seuils et du tableau pour les résultats
seuils <- seq(0.1, 6, by = 0.5)
resultats <- data.frame(seuil = numeric(), condition = character(), moyenne_vehicule = numeric(), p_value = numeric())
...
```{r, echo=FALSE, "Comparaison de la moyenne des vélos puis des véhicules en fonction du seuil de pluie pendant les heures d'affluence le SOIR (17h et 18h), en semaine et pendant la période non scolaire"}
# Initialisation des seuils et du tableau pour les résultats
seuils <- seq(0.1, 6, by = 0.5)
resultats <- data.frame(seuil = numeric(), condition = character(), moyenne_vehicule = numeric(), p_value = numeric())
...
## periode de scolaire entre 7 et 8 heure du matin
```{r}
# Initialisation des seuils et du tableau pour les résultats
seuils <- seq(0.1, 6, by = 0.5)
resultats <- data.frame(seuil = numeric(), condition = character(), moyenne_vehicule = numeric(), p_value = numeric())
...
compare_traffic_ratios <- function(data) {
  # Vérification que les colonnes nécessaires sont présentes
  required_columns <- c("date", "RR1", "bike", "vehicule")
  if (!all(required_columns %in% colnames(data))) {
    stop("Le jeu de données doit contenir les colonnes suivantes : 'date', 'RR1', 'bike', 'vehicule'")
  }
  
  # Filtrer et créer des catégories pour la pluie
  data <- data %>%
    filter(!is.na(RR1)) %>%
    mutate(pluie = ifelse(RR1 > 0, "pluie", "non pluie"))

  # Filtrer pour ne conserver que les jours où il a plu
  days_with_rain <- data %>%
    group_by(date) %>%
    filter(any(pluie == "pluie")) %>%
    ungroup()

  # Calculer les totaux horaires pour chaque condition
  totaux_horaires <- days_with_rain %>%
    group_by(date, heure, pluie) %>%
    summarise(total_bike = sum(bike, na.rm = TRUE),
              total_vehicule = sum(vehicule, na.rm = TRUE)) %>%
    ungroup()

  # Calculer les rapports pour les vélos et les voitures
  rapports_horaires <- totaux_horaires %>%
    group_by(date, heure) %>%
    mutate(rapport_bike = total_bike / sum(total_bike + total_vehicule),
           rapport_vehicule = total_vehicule / sum(total_bike + total_vehicule)) %>%
    ungroup()

  # Graphique pour les vélos avec des lignes de couleurs différentes
  plot_bike <- ggplot(rapports_horaires, aes(x = heure, y = rapport_bike, group = pluie, color = pluie)) +
    geom_point() +
    geom_line(aes(color = pluie)) +
    scale_color_manual(values = c("pluie" = "red", "non pluie" = "blue")) +
    facet_wrap(~ date, scales = "free_x") +
    labs(title = "Rapport des vélos pendant les périodes de pluie et de non pluie",
         x = "Heure", y = "Rapport des vélos") +
    theme_minimal()
...
compare_traffic_ratios(V2_meteo)
