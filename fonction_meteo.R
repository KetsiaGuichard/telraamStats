...
new_data = function(df_meteo, data_mouv){
  ...
  df_meteo = df_meteo %>% select(date,N,RR1,GLO,U,VV, T)   # Sélection des variables pertinentes de météo

  data_mouv = data_mouv %>% mutate(vehicule = car + heavy) %>%  select(date, uptime, v85, vacation, holiday, weekday, vehicule, pedestrian, type)
...
corrélation_uptime = function(complete_data, liste_var_selected =NULL){
  if( !is.null((liste_var_selected))){
    ...
    mat_var = cor(drop_na(complete_data %>%select(all_of(selected_vars))), method = "spearman")
  }else{mat_var = cor(drop_na(complete_data %>% select(!date)), method = "spearman")}
  corrplot(mat_var, type = "upper", method = "number", title= as.character(complete_data$segment_name[[1]][2]))
}
