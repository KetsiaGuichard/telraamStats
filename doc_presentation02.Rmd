---
title: "Analyse pour présentation 2.1"
author: "Kevin"
date: "`r Sys.Date()`"
output:
  rmdformats::downcute:
...
#save(V2_meteo, file = "data/V2_meteo.RData")
...
Pour la quantité de précipitation enregistrée les week-ends, on semble observer un comportement aléatoire du trafic. Des facteurs externes semblent plus influencer le trafic que les précipitations
    ...
    Même analyse en filtrant tout d'abord sur la période non scolaire (holiday=TRUE) et en semaine
...
```{r, include=FALSE, message=FALSE, warning=FALSE, "chunk 12"}

# Plus de détails

#Une autre façon de voir les choses est de regarder les proportions de vélos et de véhicules par jour et par condition.
...
V2_meteo = V2_meteo %>% mutate(segment_fullname= ifelse(segment_fullname== "", "9000006552 - Boulevard Leannec 2", segment_fullname))

# A l'aide d'un geom_point et d'un facet wrap
ggplot(V2_meteo, aes(x = day, y = vehicule)) +
  geom_point() +
...
Etrange dans ce sens où on a une significativité pour certaines différences de moyennes et pour de plus grandes différence on a une non significativité. Cela peut être dû à la taille de l'échantillon. En effet, plus on considère une grande valeur de précipitation, moins on a de données pour effectuer le test de significativité. Cela peut expliquer pourquoi on observe une non significativité pour les plus grandes valeurs de précipitation.
